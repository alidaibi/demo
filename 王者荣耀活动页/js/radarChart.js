...
function RadarFactory(element, options) {
    ...
    if (element) {
        ...
        var defaultRadius = Math.min(canvas.width, canvas.height) * 0.5 * userConfig.scale; // 正多边形的默认半径
        ...
        function drawCanvasAnimation() {
            ...
            (function drawFrame() {
                ...
                dataFillOptions = {
                    ...
                    fillStyleFunc: userConfig.dataFill.fillStyleFunc
                },
                dataCircleOptions = {
                    dataPoints: dataPointsPosArray,
                    r: userConfig.dataCircle.r,
        ...
        function drawDataLine(options) {
            ...
            for (var i = 1; i <= dataPointsPosArrayLen; i++) {
                ...
                var x1 = dataPointsPosArray[i - 1].x;
                var y1 = dataPointsPosArray[i - 1].y;

                var nextIndex = i;
                if (i == dataPointsPosArrayLen) {
                    nextIndex = 0;
                }
                var x2 = dataPointsPosArray[nextIndex].x;
                var y2 = dataPointsPosArray[nextIndex].y;
                var offset = 0;
                // console.log((y2-y1)/(x2-x1))
                var angle = Math.atan(Math.abs(y2 - y1) / Math.abs(x2 - x1));
                // console.log(i,angle*180/Math.PI)
                var x1p = x1 + offset * Math.cos(angle);
                var y1p = y1 + offset * Math.sin(angle);
                var x2p = x2 - offset * Math.cos(angle);
                var y2p = y2 - offset * Math.sin(angle);

                if (x1 > x2) {
                    x1p = x1 - offset * Math.cos(angle);

                    x2p = x2 + offset * Math.cos(angle);
                }

                if (y1 > y2) {
                    y1p = y1 - offset * Math.sin(angle);
                    y2p = y2 + offset * Math.sin(angle);
                ...
                var grad = context.createLinearGradient(x1p, y1p, x2p, y2p);
                /* 指定几个颜色 */
                grad.addColorStop(0, '#c8b590'); // 红
                grad.addColorStop(1, '#262625'); // 紫
                /* 将这个渐变设置为fillStyle */
        ...
        function drawDataFill(options) {
            ...
            if (options.fillStyleFunc(context, dataPointsPosArray)) {
                fillStyle = options.fillStyleFunc(context, dataPointsPosArray);
            }
        ...
        function drawDataCircle(options) {
            ...
            //console.log(17 * ratio)
            //context.save();
            // context.drawImage(dotImg,userConfig.origin[0]+dataPointsPosArray[i].x-(17 * ratio)/2,userConfig.origin[1]+dataPointsPosArray[i].y-(17 * ratio)/2,17 * ratio,17 * ratio)
            //context.restore();
            //}
